...
const getStatusProps = (status, file) => {
    switch (status) {
        case "progress":
            return {
                class: "upload-progress",
                component: <>
                    <img src="upload_progress.svg" alt="upload_progress" className="fade-up w-28 mb-4" />
        ...
        case "success":
            return {
                ...
                component: <>
                    <img src="upload_success.svg" alt="upload_success" className="fade-up w-28 mb-4" />
                    <p className="fade-up text-lg font-semibold mb-3">Uploaded</p>
                    {file && <p className="">{file.name}</p>}
                </>
            }
        default:
            return {
...
export default function Home() {
    ...
    const handleDrop = (event, field) => {
        ...
        if (droppedFiles.length > 0) {
            ...
            setFiles(s => {return {...s, ...{[field]: newFiles[0]}}})
    ...
    return (
        <div className="w-screen h-screen flex items-center justify-center">
            <div className="flex flex-col items-center max-w-4xl w-full text-gray-600">
                ...
                <button onClick={() => { setSubmitStatus(!submitStatus) }} className={`${!submitStatus ? "bg-[#FA4EA9]" : "progress"} cursor-pointer px-8 py-4 text-white font-semibold text-lg rounded-lg w-[251px]`}>
